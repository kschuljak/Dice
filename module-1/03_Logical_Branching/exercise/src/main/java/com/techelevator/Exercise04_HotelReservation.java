...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights) {
        if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            ...
            double weekendPriceTotal = (numOfWeekendNights * DAILY_RATE);
            double weekdayPriceTotal = ((numOfTotalNights - numOfWeekendNights) * DISCOUNT_RATE);
            return (weekendPriceTotal + weekdayPriceTotal);
        }
    }
