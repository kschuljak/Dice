...
**************************************************************************/
-- drop tables in order from most dependent to independent (opposite order of creation)
...
**************************************************************************/
-- Table customer
-- varchar for stings of varying size, specify max length
-- char for strings of fixed size, specify length
...
SELECT ps.service_date AS "Date"
	, c.name
	, c.street
	, c.city
	, c.state_code
...
JOIN service s
	ON ps.service_id = s.service_id
WHERE c.name = 'Paul McCartney'
	AND service_date = '2021-02-12'
ORDER BY p.name;
